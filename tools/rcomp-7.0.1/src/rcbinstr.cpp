...
bool ResourceDataStream::StreamOutReturningWhetherContainsCompressedUnicode(RCBinaryStream& aStream, int& aSizeWhenUncompressed, bool dump)
	{
startOfFirstPass:
	...
	for (int pass=0; pass<3; ++pass)
		...
		if (pass==2)
			...
			aStream.Write(iBuffer, numberOfBytesToFirstMark);
		...
		for (int i=0; i<numberOfMarks; ++i) // must iterate forwards so that lengthOfLastRun is correctly assigned (after the end of this loop) to point to the iOtherData (i.e. run-length) of the last EMarkType_EndOfCompressedUnicodeRun mark
			...
			if (pass==2)
				{
				aStream.Write(iBuffer+bufferIndex, numberOfBytesToNextMark);
				}
