...
//
	{
	  TInt32 xuids[KMaxCheckedUid];
	  for (int i = 0; i < KMaxCheckedUid; i++) {
	    xuids[i] = iType[i].iUid;
#ifdef __BIG_ENDIAN__
	    // For big endian we need to swap the byte order before cheeck suming
	    flipi(xuids[i]);
#endif
	  }
	return((checkSum(((BYTE*)xuids)+1)<<16)|checkSum((BYTE*)xuids));
	}
